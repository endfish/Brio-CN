    ...
    uint BaseEmoteId)
{
    ...
    {
        (CommonPoseKind.ChairSit, 0) => Localize.Text("Base Chair Sit Pose"),
        (CommonPoseKind.ChairSit, _) => Localize.Format("Chair Sit Pose {0:D2}", VariantIndex),
        (CommonPoseKind.Standing, 0) => Localize.Text("Base Standing Pose"),
        (CommonPoseKind.Standing, _) => Localize.Format("Standing Pose {0:D2}", VariantIndex),
        (CommonPoseKind.GroundSit, 0) => Localize.Text("Base Ground Sit Pose"),
        (CommonPoseKind.GroundSit, _) => Localize.Format("Ground Sit Pose {0:D2}", VariantIndex),
        (CommonPoseKind.Sleep, _) => Localize.Format("Sleep Pose {0:D2}", VariantIndex),
        _ => Localize.Format("Timeline {0}", TimelineId),
    };
}
