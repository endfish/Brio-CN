...
public class CatalogWindow : Window, IDisposable
{
    ...
    private void DrawSpawnTab()
    {
        ...
        ImGui.TextUnformatted(global::Brio.Resources.Localize.Text("Enter a game path (.mdl, .avfx, etc.)"));
        ...
        using(ImRaii.Disabled(string.IsNullOrWhiteSpace(_spawnPath)))
            if(ImGui.Button(global::Brio.Resources.Localize.Text("Spawn Object"), new Vector2(buttonWidth, 0)))
            {
    ...
    private void DrawMetadataEditor()
    {
        ...
        using(ImRaii.Disabled(selectedIndex <= 0))
            if(ImGui.Button(global::Brio.Resources.Localize.Text("Previous"), new Vector2(82 * ImGuiHelpers.GlobalScale, buttonHeight)))
                SelectMetadataItem(selectedIndex - 1);

        ImGui.SameLine();

        using(ImRaii.Disabled(selectedIndex < 0 || selectedIndex >= items.Count - 1))
            if(ImGui.Button(global::Brio.Resources.Localize.Text("Next"), new Vector2(82 * ImGuiHelpers.GlobalScale, buttonHeight)))
                SelectMetadataItem(selectedIndex + 1);
        ...
        ImGui.TextUnformatted(global::Brio.Resources.Localize.Text("Index"));
        ImGui.SameLine();
        ...
        {
            var goTo = ImGui.Button(global::Brio.Resources.Localize.Text("Go to"));
            if(items.Count > 0 && (goTo || jumpConfirmed))
                SelectMetadataItem(Math.Clamp(_metaJumpIndex, 1, items.Count) - 1);
        }

        ImGui.TextDisabled(global::Brio.Resources.Localize.Format("Item {0} of {1}", selectedIndex + 1, items.Count));
        ...
        if(ImBrio.FontIconButton("###meta_expansion_from_path", FontAwesomeIcon.FileImport,
            global::Brio.Resources.Localize.Format("Set from path ({0})", pathExpansion), !string.IsNullOrWhiteSpace(pathExpansion)))
        ...
        if(_metaKind == ObjectPathKind.VFX)
        {
            ...
            if(ImGui.InputInt(global::Brio.Resources.Localize.Text("Length###meta_length"), ref length))
    ...
    private static void DrawStringListEditor(string id, List<string> values, ref string input, string? suggestion = null)
    {
        ...
        ImGui.InputTextWithHint($"###{id}_input", global::Brio.Resources.Localize.Text("Add..."), ref input, 64);
        ...
        {
            ImGui.SameLine();
            if(ImBrio.FontIconButton($"##{id}_from_path", FontAwesomeIcon.FileImport, global::Brio.Resources.Localize.Format("Add from path ({0})", suggestion)))
            {
    ...
    private bool DrawSearchAndViewMode(ref string searchText, ref CatalogDisplayMode mode, string id, float extraReservedWidth = 0)
    {
        ...
        float reservedWidth = (buttonWidth * 2) + (spacing * 2) + (extraReservedWidth * ImGuiHelpers.GlobalScale);

        ImGui.SetNextItemWidth(ImBrio.GetRemainingWidth() - reservedWidth);
        if(ImGui.InputTextWithHint($"###{id}_search", global::Brio.Resources.Localize.Text("Search..."), ref searchText, 256))
            applay = true;
    ...
    private void DrawFurnitureFilters()
    {
        ...
        ImGui.SameLine();
        ImGui.TextUnformatted(Localize.Format("{0:N0} of {1:N0} items", _filteredFurnishings.Count, _allFurnishings.Count));
    }
    private void DrawModelFilters(bool showPreviewButton = true)
    {
        ...
        {
            ImGui.SameLine();
            if(ImBrio.IconButtonWithText(FontAwesomeIcon.Eye, Localize.Text("Open Model Preview"),
        ...
        ImGui.TextUnformatted(Localize.Format("{0:N0} of {1:N0} items", _filteredModels.Count, _allModels.Count));
    }
    private void DrawVfxFilters()
    {
        ...
        ImGui.TextUnformatted(Localize.Format("{0:N0} of {1:N0} items", _filteredVfx.Count, _allVfx.Count));
    }
    ...
    private void IconRightClick(string id, ObjectPathKind kind, string path, string name, uint iconId)
    {
        if(ImGui.BeginPopupContextItem(id))
        {
            ...
            if(ImGui.MenuItem(Localize.Text(fav ? "Remove Favorite" : "Add Favorite")))
                ToggleFav(kind, path, name, iconId);
    ...
    private void DrawModelPreviewWindow()
    {
        ...
        ImGui.TextDisabled(Localize.Format("Model {0} of {1}", _modelLivePreviewIndex + 1, _filteredModels.Count));
        ImGui.Separator();
        ...
        using(ImRaii.Disabled(_modelLivePreviewIndex <= 0))
            if(ImGui.Button(Localize.Text("Previous"), new Vector2(halfWidth, 0)))
                NavigateModelPreview(-1);

        ImGui.SameLine();
        using(ImRaii.Disabled(_modelLivePreviewIndex >= _filteredModels.Count - 1))
            if(ImGui.Button(Localize.Text("Next"), new Vector2(halfWidth, 0)))
                NavigateModelPreview(1);
    ...
    private sealed class CatalogModelPreviewWindow : Window
    {
        ...
        public CatalogModelPreviewWindow(CatalogWindow owner)
            : base($"{Localize.Text("Model Preview")}###brio_catalog_model_preview", ImGuiWindowFlags.NoCollapse)
        {
            _owner = owner;
            SizeConstraints = new WindowSizeConstraints
            {
